...
import type { LlmCallEvent } from "../agent/agent.js";
...
export class Distiller {
  constructor(
    ...
    /** LLM 调用事件回调（Phase 8：distill 轨迹/计费；agentId 恒为 'distiller'） */
    private onLlmCall?: (info: LlmCallEvent) => void
  ) {}
  ...
  async distill(input: {
    ...
    // 1. LLM 提炼
    const raw = await this.callLlm(transcript, threadId, task);
  ...
  /** LLM 调用（严格结构化标签输出；Phase 8 加调用事件） */
  private async callLlm(transcript: string, threadId: string, task?: string): Promise<string> {
    const system =
    ...
    try {
      ...
      this.fireLlmCall(threadId, start, undefined, e instanceof Error ? e.message : String(e));
      throw e;
    }
    this.fireLlmCall(threadId, start, response);

    const text = response.content.find((b) => b.type === "text");
    return text && text.type === "text" ? text.text : "";
  }

  /** 发 LLM 调用事件（agentId 恒为 'distiller'，与 Agent 的事件同构） */
  private fireLlmCall(
    threadId: string,
    start: number,
