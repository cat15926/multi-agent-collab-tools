/**
 * Phase 9 — CLI 入口（Claude Code Brain 版）
 *
 * 在 Phase 8 基础上让 Agent 的"脑+手"可替换（Brain 接口）：
 *   - config/agents/*.json 的 runtime 字段选择 brain（anthropic | claude-code）
 *   - claude-code = Claude Code 完整 agentic loop（Read/Edit/Bash/Grep 原生工具）
 *   - --brain= 全局覆盖；--cc-max-turns / --cc-budget 成本护栏
 *   - Phase 8 全部用法不变（trace/stats/kb/REPL）
 *
 * 用法：
 *   npm run phase9 -- trace [list] [N] · trace show <id|last> [--full]
 *   npm run phase9 -- stats [--by=agent|thread|day]
 *   npm run phase9 -- "任务" --pattern=pipeline --agents=a,b   # 跑完自动打回执行
 *   npm run phase9 -- "@bob ..." --brain=anthropic             # 回退裸 API
 */

import { resolve } from "path";
import { AgentRegistry, type AgentConfig } from "./registry/agent-registry.js";
...
import { AnthropicBrain } from "./agent/anthropic-brain.js";
import {
  ClaudeCodeBrain,
  CC_GUARDS_DEFAULT,
  type CcBrainDeps,
  type CcGuards,
} from "./agent/claude-code-brain.js";
...
interface CliArgs {
  ...
  // ─── Brain 可替换（Phase 9 新增）──────────────────────────────
  brain: string | null; // --brain=anthropic|claude-code（全局覆盖 cfg.runtime）
  ccMaxTurns: number | null; // --cc-max-turns=N（CC agentic loop 轮数上限，默认 15）
  ccBudget: number | null; // --cc-budget=X（CC 单次回复成本上限 $，默认 0.5）
}
...
function parseArgs(): CliArgs {
  ...
  for (const arg of args) {
    ...
    else if (arg.startsWith("--brain=")) r.brain = arg.split("=")[1];
    else if (arg.startsWith("--cc-max-turns=")) {
      const n = parseInt(arg.split("=")[1], 10);
      if (!Number.isNaN(n) && n > 0) r.ccMaxTurns = n;
      else console.warn(`⚠️  无效的 --cc-max-turns "${arg}"（已忽略）`);
    } else if (arg.startsWith("--cc-budget=")) {
      const x = parseFloat(arg.split("=")[1]);
      if (!Number.isNaN(x) && x > 0) r.ccBudget = x;
      else console.warn(`⚠️  无效的 --cc-budget "${arg}"（已忽略）`);
...
export function listAgents(registry: AgentRegistry): void {
  ...
  for (const agent of agents) {
    ...
    console.log(`     运行时: ${agent.runtime ?? "anthropic"}${agent.runtime === "claude-code" ? "（Claude Code）" : ""}`);
...
export function makeAgentOptionsFactory(
  ...
  return () => ({
    ...
    onLlmCall: (info) => {
      ...
      // Phase 9：CC brain 追加 cc_cost_usd / cc_num_turns（SDK 口径成本，挂在最后一条 assistant 事件上）
      tracer?.recordSpan(
        `llm:${info.model}`,
        "llm",
        { startTs: Date.now() - info.durationMs, endTs: Date.now() },
        {
          ...
          ...(info.ccTotalCostUsd !== undefined
            ? { cc_cost_usd: Number(info.ccTotalCostUsd.toFixed(4)) }
            : {}),
          ...(info.ccNumTurns !== undefined ? { cc_num_turns: info.ccNumTurns } : {}),
        },
        {
          agentId: info.agentId,
...
export function buildAgentFactory(
  ...
  makeBrain?: (cfg: AgentConfig, opts: AgentOptions) => Brain
  ...
  return (agentId: string): Agent => {
    ...
    if (makeBrain) opts.brain = makeBrain(cfg, opts);
    return new Agent(cfg, opts);
  };
}

/** Phase 9：CC brain 运行时依赖（含沙箱；REPL 每行重建工厂时用 getter 取最新开关状态） */
export type BrainDeps = CcBrainDeps & { sandbox: Sandbox };

/**
 * Phase 9：Brain 工厂（one-shot / REPL 共用）——按 Agent 配置的 runtime 字段选择 Brain。
 * 优先级：CLI --brain= 全局覆盖 > cfg.runtime > 默认 anthropic。
 * deps 建议传 getter 形式的快照函数（REPL 开关切换后下一行重建即生效）。
 */
export function makeBrainFactory(
  deps: BrainDeps,
  opts: { override?: string | null; guards?: CcGuards }
): (cfg: AgentConfig, agentOpts: AgentOptions) => Brain {
  const guards = opts.guards ?? CC_GUARDS_DEFAULT;
  return (cfg: AgentConfig, agentOpts: AgentOptions): Brain => {
    const runtime = opts.override ?? cfg.runtime ?? "anthropic";
    if (runtime === "claude-code") {
      return new ClaudeCodeBrain({
        ...
        deps: () => ({
          kb: deps.kb,
          kbWriteOn: deps.kbWriteOn,
          memoryOn: deps.memoryOn,
        }),
        guards,
        allowedTools: agentOpts.allowedTools, // cfg.tools 白名单 → canUseTool 内 deny
      });
    }
    if (runtime !== "anthropic") {
      throw new Error(
        `未知 runtime "${runtime}"（合法值: anthropic | claude-code）`
      );
    }
    // anthropic：从 AgentOptions 取工具上下文（与未注入 brain 的缺省路径等价）
    return new AnthropicBrain({
      model: cfg.model,
      client: agentOpts.client,
      toolRegistry: agentOpts.toolRegistry,
      allowedTools: agentOpts.allowedTools,
      sandbox: deps.sandbox,
      maxToolTurns: agentOpts.maxToolTurns,
    });
  };
}
...
export async function runPattern(params: {
  ...
  console.log(`工具调用回放: npm run phase9 -- --thread=${params.threadId} --show-tools`);
  console.log(`记忆注入回放: npm run phase9 -- --thread=${params.threadId} --show-memory\n`);
...
export function printReceipt(storage: Storage, traceId: string): void {
  ...
  console.log(
    `📊 本轮: in ${formatTokens(usage.inputTokens)} / out ${formatTokens(usage.outputTokens)} tok` +
      ` · ${costStr} · ${durS.toFixed(1)}s · 轨迹回放: npm run p9 -- trace show ${traceId}`
  );
}
...
function printUsage(): void {
  console.log(`
Phase 9 CLI — 对话 / 知识库 / 协作模式 / 可观测 / Claude Code Brain

对话:
  npm run phase9                                     # 无参数 → 交互模式（连续对话，免抄 thread ID）
  npm run phase9 -- "@agent 任务"                     # 单 Agent + 工具 + 记忆注入
  npm run phase9 -- "任务" --pattern=NAME --agents=A,B # Pattern 编排
  npm run phase9 -- "继续..." --thread=last           # 接着最近会话聊（last = 最近一条）

Brain（Phase 9）:
  --brain=anthropic|claude-code   全局覆盖（优先级 > Agent 配置 runtime 字段 > anthropic）
  --cc-max-turns=N                CC agentic loop 轮数上限（默认 15）
  --cc-budget=X                   CC 单次回复成本上限 $（默认 0.5；pricing 未配价时自动降级）
  config/agents/*.json 的 "runtime" 字段决定每个 Agent 默认跑哪个 brain

会话:
  npm run phase9 -- threads [N]                       # 最近会话列表（默认 10）
  npm run phase9 -- --thread=<id|last> --show-tools   # 工具调用回放
  npm run phase9 -- --thread=<id|last> --show-memory  # 记忆注入回放
  npm run phase9 -- --thread=<id|last> --show-workflow

可观测（Phase 8，子命令）:
  npm run phase9 -- trace [list] [N]                  # 近期轨迹列表（kind/耗时/tokens/成本/状态）
  npm run phase9 -- trace show <id|last> [--full]     # 轨迹瀑布树（route→step→agent→llm/tool）
  npm run phase9 -- stats [--by=agent|thread|day]     # token 计费聚合
  --verbose / --log-level=debug|info|warn|error       # 结构化日志 stderr 阈值
  （每轮对话/编排/提炼结束自动打 📊 回执行：tokens/成本/耗时/轨迹 id）

知识库（子命令，推荐）:
  npm run phase9 -- kb add "内容" --type=lesson --title=标题 [--keywords=a,b] [--thread=] [--agent=]
                                                     # type: decision|lesson|observation|outcome
  npm run phase9 -- kb search "词" [--type=] [--limit=N]
  npm run phase9 -- kb list [--type=] [--limit=N] · kb stats
  npm run phase9 -- kb del <id> · kb verify <id>
  npm run phase9 -- kb distill <threadId|last> [--force] [--distill-model=M]
...
示例:
  # Claude Code brain（bob 用 CC 完整 agentic loop 读项目）
  npm run p9 -- "@bob 看看 package.json 里有哪些 script"

  # 回退裸 API brain（与 Phase 8 行为一致）
  npm run p9 -- "@bob 你好" --brain=anthropic

  # Pattern + 看轨迹
  npm run p9 -- "设计配置模块" --pattern=pipeline --agents=bob,ji-tui
  npm run p9 -- trace show last
`);
}
...
async function main() {
  ...
  // ─── 仅有"运行时行为"flags（无输入、无查询/列表类动作）→ 也进 REPL ──
  // 必须在组件初始化前判断：main 会注册 Pattern，Runtime 构造再注册会撞
  // "已存在"（globalPatternRegistry 非幂等）。查询类 flags（--show-* / kb /
  // trace / stats / --list-*）走原路径，不进 REPL。
  const isQueryAction =
    args.listTools || args.listPatterns || args.listAgents ||
    args.showThreads || args.showWorkflow || args.showTools || args.showMemory ||
    args.kbAdd !== null || args.kbSearch !== null || args.kbList || args.kbDel !== null ||
    args.kbVerify !== null || args.kbStats || args.kbDistill ||
    args.traceList || args.traceShow !== null || args.statsBy !== null;
  if (!args.input && !isQueryAction) {
    const { startRepl } = await import("./repl.js");
    return startRepl({
      workdir: args.workdir ?? undefined,
      allowWrite: args.allowWrite,
      allowExec: args.allowExec,
      brainOverride: args.brain,
      ccMaxTurns: args.ccMaxTurns ?? undefined,
      ccBudget: args.ccBudget ?? undefined,
    });
  }
  ...
  // 验证输入：走到这里的无输入场景只剩查询类 flags 缺操作数（上面已拦截运行时 flags → REPL）
  if (!args.input) {
    printUsage();
    console.error("错误: 请提供输入内容（无参数运行 npm run phase9 进入交互模式）");
    process.exit(1);
  }
  ...
  // Phase 9：Brain 工厂（--brain= 覆盖 > cfg.runtime > anthropic）
  if (args.brain !== null && args.brain !== "anthropic" && args.brain !== "claude-code") {
    console.error(`错误: --brain=${args.brain} 无效（合法值: anthropic | claude-code）`);
    process.exit(1);
  }
  const makeBrain = makeBrainFactory(
    { sandbox, kb, kbWriteOn: args.allowKbWrite, memoryOn: !args.noMemory },
    {
      override: args.brain,
      guards: {
        maxTurns: args.ccMaxTurns ?? CC_GUARDS_DEFAULT.maxTurns,
        budgetUsd: args.ccBudget ?? CC_GUARDS_DEFAULT.budgetUsd,
      },
    }
  );

  /** 构建 Agent（解析 allowedTools：CLI --tools 优先，否则 Agent 配置 tools，否则全部） */
  const buildAgent = buildAgentFactory(registry, storage, makeAgentOptions, args.tools, makeBrain);
  ...
  if (!args.patternName) {
    try {
      ...
      printReceipt(storage, trace.id);
      console.log(`工具调用回放: npm run phase9 -- --thread=${result.threadId} --show-tools`);
      console.log(`记忆注入回放: npm run phase9 -- --thread=${result.threadId} --show-memory`);
      console.log(`继续此会话: npm run phase9 -- "继续" --thread=${result.threadId}\n`);
    } catch (error) {
