...
export class A2AHandler {
  ...
  constructor(
    ...
    /** Phase 8：a2a span（每跳一记；由 Router 注入） */
  ...
  async handle(
    ...
    while (currentDepth < this.config.maxDepth) {
      ...
      // Phase 8：a2a span（每跳一记；被唤醒 Agent 的 llm/tool 子 span 挂其下）
      const agentReply = await withSpan(
        this.tracer,
        `a2a:${currentAgentId}→${nextAgentId}`,
        "a2a",
        async (span) => {
          span.setAttribute("from", currentAgentId);
          span.setAttribute("to", nextAgentId);
          span.setAttribute("hop", currentDepth);
          const out = await nextAgent.reply(input, {
            threadId: context.threadId,
