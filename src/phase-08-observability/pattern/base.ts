...
export abstract class BasePattern implements Pattern {
  ...
  /**
   * 执行单个 Agent 调用（Phase 8：咽喉点开 agent span，llm/tool 子 span 自动挂其下）
  ...
  protected async executeAgent(
    ...
    try {
      // 调用 Agent（包 agent span；并行扇出时各分支上下文在开 span 那刻各自快照，互不串扰）
      const output = await withSpan(
        context.tracer,
        `agent:${agent.id}`,
        "agent",
        async (span) => {
          span.setAttribute("step_number", stepNumber);
          span.setAttribute("role", opts?.role ?? "collaborator");
          span.setAttribute("input_preview", input.slice(0, 200));
          if (context.executionId) span.setAttribute("execution_id", context.executionId);
          const out = await agent.reply(input, {
            threadId: context.threadId,
            participants: context.agents.map((a) => a.id),
            history: (context.history || []) as any, // PatternHistoryItem compatible with Message
            hasMention: false,
            role: opts?.role,
            memoryContext: context.memory, // Phase 7：长期记忆透传（唯一咽喉点）
          });
