 ...
 *
 * Phase 8 新增：onLlmCall 事件——每次 messages.create 后发 LlmCallEvent
 * （token usage / 延迟 / stop_reason / 轮次），镜像 Phase 6 的 onToolCall 先例；
 * 落盘（llm span + 计费）由 CLI 工厂注入回调完成，Agent 自身不依赖观测层。
 *
 * 职责：
 * - 接受输入并生成回复
 * - 多 Agent 协作感知
...
export class Agent {
  ...
  async reply(content: string, options: AgentReplyOptions): Promise<string> {
    ...
    // 无工具 → 原单次调用逻辑（向后兼容）
    if (tools.length === 0) {
      return this.replyWithoutTools(llmMessages, systemPrompt, options);
    }
  ...
  private async replyWithoutTools(
    ...
    try {
      ...
      this.fireLlmCall(options, 1, start, undefined, e instanceof Error ? e.message : String(e));
      throw e;
    }
    this.fireLlmCall(options, 1, start, response);
  ...
  private async replyWithTools(
    ...
    for (let turn = 0; turn < this.maxToolTurns; turn++) {
      ...
      this.fireLlmCall(options, turn + 1, start, response);
