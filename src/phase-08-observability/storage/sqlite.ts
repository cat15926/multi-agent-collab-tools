...
// ─── 可观测类型（Phase 8 新增：traces + spans 双表，mini-OTel Span 树）──────────

/** Trace 种类：chat 单轮 / pattern 编排 / distill 提炼 */
export type TraceKind = "chat" | "pattern" | "distill";
...
export type SpanKind =
  | "route" // 路由决策（targets/fallback）
  | "kb" // 记忆注入（entry_ids/命中数）
  | "step" // 编排步骤（链接 execution_id）
  | "agent" // 一次 reply（含 llm/tool 子 span）
  | "llm" // 一次 messages.create（tokens/model/stop_reason/turn）
  | "tool" // 一次工具执行（链接 tool_call_id）
  | "a2a" // handoff 跳转
  | "distill"; // 蒸馏反思
...
/** Span 记录（树节点；attributes 为自由指标/preview/链接 id） */
...
/** 新建 Trace 的选项（id 由 Tracer 生成——span 需在插入前引用它） */
export interface AddTraceOptions {
  id: string;
  kind: TraceKind;
  entry: string;
  threadId?: string;
...
/** 新建 Span 的选项（span 结束时才落盘，duration 此刻已知） */
export interface AddSpanOptions {
  id: string;
  traceId: string;
  parentId?: string;
  kind: SpanKind;
...
/** llm span 用量聚合行（按 分组键 × 模型；成本在读时由 pricing 计算） */
export interface LlmUsageRow {
  key: string; // agent_id / thread_id / 'YYYY-MM-DD'
  model: string;
  calls: number;
  inputTokens: number;
  outputTokens: number;
...
export class Storage {
  ...
  /** 新建 trace（status=running；结束时 finishTrace 收口） */
  addTrace(opts: AddTraceOptions): Trace {
    this.db
      .prepare(`
        INSERT INTO traces (id, kind, entry, thread_id, title, status, started_at)
        VALUES (?, ?, ?, ?, ?, 'running', ?)
      `)
      .run(
        opts.id,
  ...
  /** 结束 trace（写 ended_at + 终态） */
  finishTrace(id: string, status: TraceStatus, error?: string): void {
    this.db
      .prepare(`
  ...
  getTrace(id: string): Trace | null {
    const row = this.db
      .prepare(`
        SELECT id, kind, entry, thread_id as threadId, title, status, error,
  ...
  /** 最近一条 trace（`trace show last` 用；rowid tiebreak 保证同毫秒排序确定） */
  getLastTrace(): Trace | null {
    const row = this.db
      .prepare(`
        SELECT id, kind, entry, thread_id as threadId, title, status, error,
               started_at as startedAt, ended_at as endedAt
        FROM traces ORDER BY started_at DESC, rowid DESC LIMIT 1
      `)
      .get() as any;
    return row ? this.rowToTrace(row) : null;
  }
  ...
  listTraces(limit = 20): Trace[] {
    const rows = this.db
      .prepare(`
        SELECT id, kind, entry, thread_id as threadId, title, status, error,
               started_at as startedAt, ended_at as endedAt
        FROM traces ORDER BY started_at DESC, rowid DESC LIMIT ?
      `)
      .all(limit) as any[];
  ...
  /** 落盘一个已结束的 span（结束时一次性 INSERT，duration 此刻已知） */
  addSpan(opts: AddSpanOptions): void {
    this.db
      .prepare(`
        INSERT INTO spans (id, trace_id, parent_id, kind, name, agent_id, status, error,
      ...
      .run(
        ...
        opts.startTs,
        opts.endTs,
        opts.endTs - opts.startTs,
        opts.attributes ? JSON.stringify(opts.attributes) : null
      );
  }

  /** 某 trace 的全部 spans（按开始时间排序；树装配在 trajectory.ts） */
  getSpansByTrace(traceId: string): SpanRecord[] {
    const rows = this.db
      .prepare(`
        SELECT id, trace_id as traceId, parent_id as parentId, kind, name,
               agent_id as agentId, status, error, start_ts as startTs,
               end_ts as endTs, duration_ms as durationMs, attributes
  ...
  /** llm span 用量聚合（按 分组键 × 模型；成本读时计算——见 pricing.ts） */
  summarizeLlmUsage(by: "agent" | "thread" | "day", limit = 20): LlmUsageRow[] {
    const keyExpr =
      by === "agent"
        ? "COALESCE(s.agent_id, '(unknown)')"
        : by === "thread"
          ? `COALESCE(t.thread_id, '(no-thread)')`
          : `date(s.start_ts / 1000, 'unixepoch', 'localtime')`;
    const joinTraces = by === "thread" ? "JOIN traces t ON t.id = s.trace_id" : "";
    const rows = this.db
      .prepare(`
        SELECT ${keyExpr} as key,
               COALESCE(json_extract(s.attributes, '$.model'), '(unknown)') as model,
               COUNT(*) as calls,
               SUM(COALESCE(json_extract(s.attributes, '$.input_tokens'), 0)) as inputTokens,
               SUM(COALESCE(json_extract(s.attributes, '$.output_tokens'), 0)) as outputTokens
        FROM spans s
        ${joinTraces}
        WHERE s.kind = 'llm'
        GROUP BY key, model
        ORDER BY MIN(s.start_ts) DESC
        LIMIT ?
      `)
      .all(limit) as any[];
    return rows.map((r) => ({
  ...
  /** 某 trace 的 llm 用量（回执行/轨迹汇总用；含按模型明细——各模型单价不同，成本须分模型算） */
  getTraceLlmUsage(traceId: string): {
    calls: number;
    inputTokens: number;
    outputTokens: number;
    ...
    const rows = this.db
      .prepare(`
        SELECT COALESCE(json_extract(attributes, '$.model'), '(unknown)') as model,
               COUNT(*) as calls,
               SUM(COALESCE(json_extract(attributes, '$.input_tokens'), 0)) as inputTokens,
               SUM(COALESCE(json_extract(attributes, '$.output_tokens'), 0)) as outputTokens
        FROM spans
        WHERE trace_id = ? AND kind = 'llm'
        GROUP BY model
      `)
      .all(traceId) as any[];
    ...
    for (const r of rows) {
      byModel[r.model] = { calls: r.calls, inputTokens: r.inputTokens ?? 0, outputTokens: r.outputTokens ?? 0 };
      calls += r.calls;
      inputTokens += r.inputTokens ?? 0;
      outputTokens += r.outputTokens ?? 0;
    }
    return { calls, inputTokens, outputTokens, byModel };
  }
