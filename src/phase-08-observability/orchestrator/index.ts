...
export interface OrchestratorConfig {
  ...
  kb?: KnowledgeBase;
  /** 观测器（Phase 8；注入 PatternContext，executeAgent 咽喉点开 agent span） */
  tracer?: Tracer;
}
...
export class Orchestrator {
  ...
  async executePattern(params: {
    ...
    try {
      ...
      //    Phase 8：检索包 kb span（注入是耗时可观的一次动作，值得出现在轨迹里）
      ...
      if (memory && memory.length > 0) {
        ...
        this.config.tracer?.recordSpan(
          "kb:buildMemoryContext",
          "kb",
          { startTs: kbStart, endTs: Date.now() },
          { consumer: `pattern:${patternName}`, entries: memory.length }
        );
        try {
      ...
      const context: PatternContext = {
        ...
        events,
        memory,
        tracer: this.config.tracer,       // Phase 8：透传（executeAgent 咽喉点）
        executionId,                      // Phase 8：span attrs 链接 workflow_executions 行
      };
