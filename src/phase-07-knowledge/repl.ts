 ...
 * Phase 7 — REPL 交互模式
 *
 * `npm run phase7`（无参数）进入。解决 one-shot 的最大摩擦：
...
async function handleLine(rt: Runtime, line: string, state: { threadId: string | null }): Promise<boolean> {
    ...
    switch (cmd) {
      ...
      case "pattern": {
        ...
        try {
          ...
          const makeAgentOptions = makeAgentOptionsFactory(rt.storage, rt.toolRegistry, rt.sandbox);
          const buildAgent = buildAgentFactory(rt.registry, rt.storage, makeAgentOptions, []);
          await runPattern({
            patternName: name,
            task,
            threadId: tid,
  ...
  rt.currentThreadId = tid;
  const makeAgentOptions = makeAgentOptionsFactory(rt.storage, rt.toolRegistry, rt.sandbox);
  const buildAgent = (agentId: string): Agent => buildAgentFactory(rt.registry, rt.storage, makeAgentOptions, [])(agentId);

  try {
    const result = await rt.router.route(input, tid, buildAgent);
 ...
 */
export async function startRepl(opts: { workdir?: string; allowWrite?: boolean; allowExec?: boolean } = {}): Promise<void> {
