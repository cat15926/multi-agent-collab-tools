...
1. **架构决策 (ADR)**: 设计方向、技术选型、架构权衡
2. **Bug 修复复盘**: 重要问题的修复记录，包含根因分析和经验总结
...
1. 复制 [`_template-bugfix.md`](./_template-bugfix.md)
2. 命名 `004-fix-px-xxx-短描述.md`（编号递增）
3. 填写根因分析、修复方案和经验总结
4. **每次修复重要 Issue 都要记录**，积累经验库
...
| [ADR-007](./007-fix-p5-001-workflow-time-display.md) | P5-001 `--show-workflow` 时间显示错乱（毫秒被当秒×1000 + 耗时单位错误） | 5 | 2026-08-08 | ✅ accepted |
